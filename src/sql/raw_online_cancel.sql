...
declare start_date Date default '2026-04-03'; -- the test launching date

CREATE TABLE IF NOT EXISTS `gannett-datascience.test_results_zone.ss_test_online_cancel_raw`(
    event_date DATE,
    anonymous_id STRING,
    website_id STRING,
    entered_acc_mng INT64,
    confirmed_cancel INT64,
    success_save INT64,
    id_subscrip INT64
)
PARTITION BY event_date;

MERGE INTO `gannett-datascience.test_results_zone.ss_test_online_cancel_raw` AS target
USING (
  with raw as (
    select
      event_date,
    ...
    where event_date >= (
      SELECT COALESCE(MAX(event_date), start_date) 
      FROM `gannett-datascience.test_results_zone.ss_test_online_cancel_raw`)
...
WHEN NOT MATCHED THEN
    INSERT (event_date, anonymous_id, website_id, entered_acc_mng, confirmed_cancel, success_save, id_subscrip)
    VALUES (source.event_date, source.anonymous_id, source.website_id, source.entered_acc_mng, source.confirmed_cancel, source.success_save, source.id_subscrip);