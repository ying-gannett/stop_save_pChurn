...
def run_assessment(file_path: str, run_date: str, sql_file: str, target_table: str) -> None:
    """Performs data quality checks and logs results."""
    ...
    history = [r for r in load_history() if r.get('sql_file') == current_sql_filename and r.get('row_count') is not None]
    ...
    if not history:
        ...
        if not row_count_passed: print(row_alert)
        else: print("✅ Row count is within expected bounds.")
            
        if not nulls_passed: print(null_alert)
        else: print("✅ Null percentage is within expected bounds.")

    log_run(run_date, sql_file, target_table, current_row_count, current_null_pct, row_count_passed, nulls_passed)
